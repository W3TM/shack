...
Since I run a receive-only I-Gate that transmits received packets to APRS-IS already, I do not typically enable APRS-IS digipeating on the IC-705/MacBook Pro set-up from my home office.

This means that the station can only hear received RF packets, and does not transmit via RF or forward received packets via the Internet to APRS.IS.
...
2. Select the APRS/Packet Memory Group (04) on the rig.
...
Setting the virtual machine mic level to ~80 seems to work well.
